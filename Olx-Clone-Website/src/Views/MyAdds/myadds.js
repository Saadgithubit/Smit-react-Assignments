            ...
            {adds.map((item, index) => {
                return <div key={index} className="w-full my-3 flex justify-center flex-col sm:flex-row sm:justify-between border-2 p-3">
                    ...
                    <div className="flex flex-col w-full sm:w-auto">
                        <button className="w-full sm:w-36 h-12 my-2 m-auto font-bold bg-slate-200 hover:bg-gray-700 hover:text-white">Edit</button>
                        <button onClick={() => deleteAdd(item)} className="w-full sm:w-36 h-12 my-2 m-auto font-bold bg-slate-200 hover:bg-gray-700 hover:text-white">Delete</button>
                    </div>
                </div>
            })}
        </div>
